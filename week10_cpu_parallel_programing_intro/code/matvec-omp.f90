...
! NAME
!
!     matvec-omp.f90 - matrix-vector multiplication (fortran; openmp)
!
! DESCRIPTION
!
!     Performs a parallel matrix-vector multiplication (y=Ax) operation 
!     between a Hilbert matrix (A) and an all-ones vector (x) of 
!     dimension n in Fortran with OpenMP. The resultant vector (y) is 
!     checked by computing the sum of its elements.
!
! USAGE
!
!     gfortran -O3 -fdefault-real-8 -fopenmp matvec-omp.f90 -o matvec-omp.x
!     export OMP_NUM_THREADS=2
!     time -p ./matvec-omp.x 32768
!
! AUTHOR
!
!     Marty Kandes, Ph.D.
